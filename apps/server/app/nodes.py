        ...
        params=[
            {
                ...
                "type": "code",
                "required": True,
                "description": "Python: def transform(items): return items",
            }
        ],
    ),
    NodeDefinition(
...
def _run_code(code: str, items: Items, result_queue: multiprocessing.Queue) -> None:
    ...
    try:
        exec(code, {"__builtins__": safe_builtins, "print": safe_print}, local_vars)
        transform = local_vars.get("transform")
        if not callable(transform):
            raise RuntimeError("Define a transform(items) function")
