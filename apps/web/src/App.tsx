...
import type { NodeDefinition, WorkflowNode } from '@pab/shared';
...
export default function App() {
  ...
  const nodeTypes = useMemo(() => {
    const entries = nodeDefs.map((definition) => [definition.type, NodeRenderer]);
    return Object.fromEntries(entries) as Record<string, typeof NodeRenderer>;
  }, [nodeDefs]);

  useEffect(() => {
    const loadNodes = async () => {
      const { data } = await axios.get<{ nodes: NodeDefinition[] }>('/api/nodes');
      setNodeDefs(data.nodes);
    };
    loadNodes();
  }, []);
  ...
  const handleAddNode = (type: string) => {
    const definition = nodeDefs.find((node) => node.type === type);
    if (!definition) {
      return;
    }
    ...
    <ReactFlowProvider>
      <div className="app">
        ...
        <div className="content">
          <NodePalette nodes={nodeDefs} onAddNode={handleAddNode} />
